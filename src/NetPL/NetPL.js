...
export default class extends Component {
    ...
    render() {
        const {value, requestEarlyExitAll, profitRenderer, profitField, progressField, currencyField} = this.props;

        if (!value || !value.length) return null;

        const {selectedCurrency, targetElement} = this.state;
        let selectedValue = value.find(item => item.currency === selectedCurrency) || value[0],
            profit = selectedValue[profitField],
            progress = selectedValue[progressField];

        return (
            <div className={classNames('cb-net-pl', {loss: profit < 0})}>
                <div>
                    <span className={classNames({'cb-net-pl__link': value.length > 1})} onClick={this.displayCurrencyMenu}>NET P/L{value.length > 1 ? `, ${selectedCurrency} ` : ' '} </span>
                    {value.length > 1 ?
                        <Dropdown
                ...
                {progress ? <span className="cb-net-pl__progress-value">{progress > 0 ? '+'+progress : progress}<span className="percent">%</span></span> : null}
            </div>
        )
    }
}
