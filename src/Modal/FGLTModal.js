import React from 'react';
import Modal from './Modal';
import ModalHeader from './ModalHeader';
import ModalContent from './ModalContent';
import ModalActions from "./ModalActions";

export default function FGLTModal({currencyConfig, disableTopUp, onTopUp, alertLabel, submitButtonLabel, closeButtonLabel, title, ...props}) {
    if (!currencyConfig) return null;
    return (
        <Modal
            className="cb-FGLTModal"
            {...props}
        >
            <ModalHeader>
                {title}
            </ModalHeader>
            <ModalContent>
                <div>
                    {currencyConfig.title} {alertLabel}
                </div>
            </ModalContent>
            <ModalActions>
                <button className="cb-Button primary" onClick={!disableTopUp ? onTopUp : null} disabled={disableTopUp}>{submitButtonLabel}</button>
                <button className="cb-Button" onClick={props.onClose}>{closeButtonLabel}</button>
            </ModalActions>
        </Modal>
    )
};

FGLTModal.defaultProps = {
    alertLabel: 'balance should reach 50 before you can top up your account',
    submitButtonLabel: 'TOP UP',
    closeButtonLabel: 'Close',
    title: 'Top Up FGLT'
};
