...
export const Tooltip = React.forwardRef((props: any, ref: any) => {
  const {
    arrow = false, // eslint-disable-line
    children,
    describeChild = false,
  ...
  const [arrowRef, setArrowRef] = React.useState(null); // eslint-disable-line
  ...
  const popperRef: any = React.useRef();
  ...
  if (process.env.NODE_ENV !== 'production') {
    childrenProps['data-cb-internal-clone-element'] = true;

    // eslint-disable-next-line react-hooks/rules-of-hooks
    React.useEffect(() => {
      if (childNode && !childNode.getAttribute('data-cb-internal-clone-element')) {
        console.error(
          [
