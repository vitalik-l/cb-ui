...
export default class EEButton extends PureComponent {
  ...
  getInnerContent(key) {
    ...
    if (orderIsClosed && withTie && value === 0 && labelTie) {
      return (
        <h6
          className={classNames('ee-value', {
            [animationClassName]: shouldAnimate,
            tradeEnded: orderIsClosed,
          })}
          key={key}
        >
          {labelTie}
  ...
  renderLabel() {
    ...
    if (value || value === 0) {
      label = animationKey
        ? [animationKey].map((key) => this.getInnerContent(key))
          ...
          <defs>
            ...
            >
              <image
                ...
                width="100%"
                height="100%"
                href={imageRedGradient}
                className={classNames({ reverse: reverseLoss })}
              />
            </pattern>
          </defs>
          <circle
